...
```{r Dom~ColorMorph}
results=table(Symcap$Dom, Symcap$df$Majority)
chisq.test(results)
...
results=table(Symcap$Mix, Symcap$df$Majority)
chisq.test(results)
prop.table(results, margin = 2)
par(mar=c(4, 4, 2, 6))
...
Symcap$Reef.Area <- ifelse(Symcap$Reef.Area!="Top", yes = "Slope", no = "Top")
results=table(Symcap$df$Majority, Symcap$Reef.Area)
chisq.test(results)
prop.table(results, margin = 2)
par(mar=c(4, 4, 2, 6))
...
```{r Color~BayArea}
results=table(merged$df$Majority, merged$Bay.Area)
prop.table(results, margin = 2)
chisq.test(results)
```
...
results=table(Symcap$df$Majority, Symcap$Reef.ID)
chisq.test(results)
```
...
propCol=table(Symcap$df$Majority, Symcap$Reef.ID)
...
```{r, results='hide'}
merged$Color <- ifelse(merged$df$Majority=="orange", 1, 0)
...
manteltable = table(merged$Dom, merged$df$Majority, merged$Reef.ID)
nc <- aggregate(interaction(merged$df$Majority, merged$Dom), 
...
merged$ColDom <- interaction(merged$df$Majority, merged$Dom)
...
results=table(Symcap$df$Majority, Symcap$Reef.Type)
chisq.test(results)
```
...
merged$Color <- ifelse(merged$df$Majority=="orange", 0, 1)
...
table(merged$Dom, merged$df$Majority, merged$Reef.Area)
model=aov(Dominant~df$Majority*Reef.Area, data = merged)
Anova(model, type = 2)
```
...
model1=lm(Dominant~df$Majority*newDepth, data = merged)
Anova(model1, type = 2)
```
...
```{r, warning=FALSE}
df <- subset(merged, Reef.Type=="Patch")
dfo <- subset(df, df$Majority=="orange")
...
df <- subset(merged, Reef.Type=="Patch")
dfb <- subset(df, df$Majority=="brown")
...
df <- subset(merged, df$Majority=="orange")
...
abline(h = 0.5, lty=2)
df <- subset(merged, df$Majority=="brown")
results=glm(Dominant~newDepth, family = "binomial", data = df)
newdata <- list(newDepth=seq(0,11,0.01))
fitted <- predict(results, newdata = newdata, type = "response")
...
#results=table(merged$Dominant2, merged$df$Majority)
#chisq.test(results)
#prop.table(results, margin = 2)
