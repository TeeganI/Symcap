...
# Prevalence of symbionts and color morphs
...
The number of brown and orange colonies that were dominated by clade C or D.
```{r Dom~ColorMorph}
...
plot(effect("newDepth:Reef.Type", dom), type="response", ylab="Prob. D-dominance", multiline=T, ci.style="bands")
...
anova(cm, test="Chisq") 
# Plot interaction effects
plot(effect("newDepth:Reef.Type", cm), type="response", ylab="Prob. Color", multiline=T, ci.style="bands")
# Get fitted values
...
with(subset(int.fit, Color.Morph=="Brown"), seq(0,11,0.1)[which.min(abs(fit-0.5))])
...
## Combined color-symbiont phenotype
```{r}
...
## Figure X: Depth effects at Patch vs. Fringe reefs
```{r}
### Plotting 
