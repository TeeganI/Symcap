...
# Overall prevalence of color-symbiont combinations
```{r}
prop.table(table(Symcap.f$ColDom))
```


# Analyzing depth gradients in symbiosis and color
...
results=glm(Color~newDepth, family = "binomial", data = Symcap.f)
anova(results, test = "Chisq")
Color <- subset(Symcap.f, !is.na(newDepth) & !is.na(Color))
...
par(new = T)
par(mar=c(4.2, 4, 2, 6))
Symcap.f$Color2 <- ifelse(Symcap.f$Color=="0", 1, 0)
results=glm(Color2~newDepth, family = "binomial", data = Symcap.f)
fitted <- predict(results, newdata = list(newDepth=seq(0,11,0.01)), type = "response")
...
df <- subset(Symcap.f, Color.Morph=="Orange")
...
# Brown colonies
df <- subset(Symcap.f, Color.Morph=="Brown")
results=glm(Dominant~newDepth, family = "binomial", data = df)
newdata <- list(newDepth=seq(0,11,0.01))
fitted <- predict(results, newdata = newdata, type = "response")
...
Symcap.f$Dominant <- ifelse(Symcap.f$Dom=="C", 0, 1)
Symcap.f$Dominant2 <- ifelse(Symcap.f$Dom=="C", 1, 0)
Symcap.f$Color <- ifelse(Symcap.f$Color.Morph=="Orange", 0, 1)
results=table(Symcap.f$Color, Symcap.f$DepthInt)
results
props <- prop.table(results, margin = 2)
...
# Bay Area
## Dominant symbiont
modd <- glm(Dominant ~ newDepth * Bay.Area, family="binomial", Symcap.f)
anova(modd, test="Chisq")  # Significant Bay.Area * depth interaction
plot(effect("newDepth:Bay.Area", modd), type="response", ylab="Prob. D-dominance")
## Color Morph
modd <- glm(Color.Morph ~ newDepth * Bay.Area, family="binomial", Symcap.f)
anova(modd, test="Chisq")  # Not significant

# Reef Type
## Dominant symbiont
modd <- glm(Dominant ~ newDepth * Reef.Type, family="binomial", Symcap.f)
anova(modd, test="Chisq")  # Significant Reef.Type * depth interaction
plot(effect("newDepth:Reef.Type", modd), x.var="newDepth", type="response")
## Color Morph
modd <- glm(Color.Morph ~ newDepth * Reef.Type, family="binomial", Symcap.f)
anova(modd, test="Chisq")  # Not significant
plot(effect("newDepth:Reef.Type", modd), x.var="newDepth", type="response")

# Adjust symbiont-color prevalence across reefs for depth effect
...
actprops <- data.frame(Reef.ID=as.character(newdat[,1]),
...
# Test for differences in distribution among bay regions, reefs, and reef types
mod.null <- multinom(ColDom~newDepth, Symcap.f)
mod.bay <- multinom(ColDom~newDepth+Bay.Area, Symcap.f)
mod.bay2 <- multinom(ColDom~newDepth*Bay.Area, Symcap.f)
mod.reef <- multinom(ColDom~newDepth+Reef.ID, Symcap.f)
mod.type <- multinom(ColDom~newDepth+Reef.Type, Symcap.f)
mod.type2 <- multinom(ColDom~newDepth*Reef.Type, Symcap.f)

anova(mod.bay2, mod.bay) # Bay area * depth is significant
anova(mod.null, mod.reef) # No effect of reef id
anova(mod.type2, mod.type) # Reef type * depth is significant



## Location*Depth interactions with Multinomial models
depthdat <- expand.grid(newDepth = seq(0,12,0.5), Reef.Type=c("Patch", "Fringe"))
depthpred <- predict(mod.type2, newdata=depthdat, "probs")
depthpred
stackpoly(depthpred[1:25,c(4,2,3,1)], stack=T, main="Patch", col=rev(c("burlywood4","burlywood3","darkorange3","darkorange")),
          ylim=c(0,1), xaxlab=seq(0,12,0.5), xlab="Depth", ylab="Probability")

stackpoly(depthpred[26:50,c(4,2,3,1)], stack=T, main="Fringe", col=rev(c("burlywood4","burlywood3","darkorange3","darkorange")),
          ylim=c(0,1), xaxlab=seq(0,12,0.5), xlab="Depth", ylab="Probability")
```
