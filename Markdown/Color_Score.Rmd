...
df$majority <- apply(df[,-1], 1, function(x) names(table(x))[which.max(table(x))])
df$NAG <- apply(df[,-c(1,7)], 1, function(x) max(table(x)))
```


# Set Color Morph Column to New Data Frame Column
```{r}
# I THINK THIS IS WHERE YOUR CODE WENT WRONG -- HERE YOU SET THE SYMCAP$COLOR.MORPH COLUMN TO EQUAL THE COLOR MAJORITY COLUMN, BUT THE COLONIES (ROWS) WERE NOT IN THE SAME ORDER
#Symcap <- subset(Symcap, Colony!="7")
#Symcap$Color.Morph <- df$Majority
#merged <- subset(merged, Colony!="7")
#merged$Color.Morph <- df$Majority

# WHAT YOU NEEDED TO DO IS MERGE THE DATA FRAMES SO THE COLONIES MATCH UP
Symcap <- merge(Symcap, df[,c("Colony", "majority", "NAG")], all=T)
Symcap$Color.Morph <- Symcap$majority
merged <- merge(merged, df[,c("Colony", "majority", "NAG")], all=T)
merged$Color.Morph <- merged$majority

# WHY IS THERE A COLONY 7??
Symcap <- subset(Symcap, Colony!="7")
merged <- subset(merged, Colony!="7")
```
...
```{r}
Symcap$Reef.Area <- ifelse(Symcap$Reef.Area!="Top", yes = "Slope", no = "Top")
# HERE IS ALSO WHERE THINGS WENT WRONG - YOU WERE USING NAG FROM df BUT REEF.AREA FROM SYMCAP -- COLONY ROWS ARE NOT THE SAME IN THESE TWO DATA FRAMES! NEED TO MERGE (NOW DONE ABOVE) AND WORK WITH COLUMNS FROM THE SAME DATA FRAME!
#results=table(df$NAG, Symcap$Reef.Area)
results=table(Symcap$NAG, Symcap$Reef.Area)
chisq.test(results)
prop.table(results, margin = 2)
par(mar=c(4, 4, 2, 6))
...
```{r}
#results=table(df$NAG, Symcap$Bay.Area)  # SAME PROBLEM HERE...
results=table(Symcap$NAG, Symcap$Bay.Area)
chisq.test(results)
prop.table(results, margin = 2)
par(mar=c(4, 4, 2, 6))
...
barplot(t(prop.table(results, margin = 1)), col = c("gray10", "gray100"), xlab = "Color Morph", ylab = "Dominant Symbiont")
...
```{r}
#results=table(df$NAG, Symcap$Dom) #SAME PROBLEM HERE
results=table(Symcap$NAG, Symcap$Dom)
chisq.test(results)
prop.table(results, margin = 2)
par(mar=c(4, 4, 2, 6))
...
```{r}
#results=table(df$NAG, Symcap$Mix)  #SAME PROBLEM HERE
results=table(Symcap$NAG, Symcap$Mix)
chisq.test(results)
prop.table(results, margin = 2)
par(mar=c(4, 4, 2, 6))
...
```{r}
merged$PropInt <- cut(merged$propD, breaks = 10)
#results=table(df$NAG, merged$PropInt) #SAME PROB HERE
results=table(merged$NAG, merged$PropInt)
results
props <- prop.table(results, margin = 2)
par(mar=c(4, 4, 2, 6), lwd = 0.25)
barplot(props, col = c("gray10", "gray50", "gray100"), 
        xlab = "Proportion of D", ylab = "Proportion of colonies",
        space = 0, xaxs="i", yaxs="i", axisnames = FALSE)
axis(side=1, at=seq(0,10,1), labels=seq(0,1,0.1))
par(lwd=1)
...
```{r}
results=table(Symcap$NAG, Symcap$Reef.ID)
chisq.test(results)
prop.table(results, margin = 2)
barplot(results)
```
...
```{r}
results=table(Symcap$NAG, Symcap$Color.Morph)
chisq.test(results)
prop.table(results, margin = 2)
par(mar=c(4, 4, 2, 6))
...
## Color Morph Uncertainty Across Depths
```{r}
merged$DepthInt <- cut(merged$newDepth, breaks = 0:13)
results=table(merged$NAG, merged$DepthInt)
results
props <- prop.table(results, margin = 2)
par(mar=c(4, 4, 2, 6), lwd = 0.25)
barplot(props[,1:11], col = c("gray10", "gray50", "gray100"), 
        xlab = "", ylab = "",
        space = 0, xaxs="i", yaxs="i", axisnames = FALSE)
par(lwd=1)
legend("topright", legend=c("3 Agreements", "4 Agreements", "5 Agreements"), fill=c("gray10", "gray 50", "gray100"), inset = c(-.22, 0), xpd = NA)
...
df <- subset(merged, merged$Color.Morph=="Orange")
results=glm(factor(Dom)~newDepth, family = "binomial", data = df)
